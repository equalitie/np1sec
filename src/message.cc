...
std::string np1secMessage::session_view_as_string(){
  std::string output = c_np1sec_delim;
  for (std::vector<UnauthenticatedParticipant>::iterator it = session_view.begin(); it != session_view.end(); ++it){
    ...
    //length 
    //output += (*it).long_term_pub_key_hex + c_np1sec_delim; 
  }
...
void np1secMessage::string_to_session_view(std::string sv_string) {
  ...
  while (!token.empty()) {
    string nickname = token.substr(0, token.find(c_subfield_delim.c_str()));
    string fingerprint = token.substr(nickname.length(), nickname.length() + c_fingerprint_length);
...
void np1secMessage::format_generic_message() {
  ...
  sys_message = "" + std::to_string(message_type) + c_np1sec_delim;
  ...
  sys_message += signature + c_np1sec_delim;
...
void np1secMessage::unwrap_generic_message() {
  ...
  std::string message = base64_decode(strtok(NULL, c_np1sec_delim));
  message_type = (np1secMessageType)atoi(strtok(&message[0], c_np1sec_delim));
  std::string temp = strtok(NULL, c_np1sec_delim);
  ...
  if (!temp.empty()) {
    ...
    switch (message_type) {
      case PARTICIPANTS_INFO:
        sv_string = strtok(NULL, c_np1sec_delim);
        key_confirmation = strtok(NULL, c_np1sec_delim);
        z_sender = strtok(NULL, c_np1sec_delim);
        signature = strtok(NULL, c_np1sec_delim);
        string_to_session_view(sv_string);
        break;
      case SESSION_CONFIRMATION:
        sv_string = strtok(NULL, c_np1sec_delim);
        session_key_confirmation = strtok(NULL, c_np1sec_delim);
        signature = strtok(NULL, c_np1sec_delim);
        string_to_session_view(sv_string);
        break;
      case JOIN_REQUEST:
        joiner_info = strtok(NULL, c_np1sec_delim);
        signature = strtok(NULL, c_np1sec_delim);
        break;
      case JOINER_AUTH:
        key_confirmation = strtok(NULL, c_np1sec_delim);
      ...
      case FAREWELL:
        sv_string = strtok(NULL, c_np1sec_delim);
        z_sender = strtok(NULL, c_np1sec_delim);
        meta_message = strtok(NULL, c_np1sec_delim);
        signature = strtok(NULL, c_np1sec_delim);
        string_to_session_view(sv_string);
        break;
      case USER_MESSAGE:
...
void np1secMessage::unwrap_user_message() {
  ...
  encrypted_message = strtok(NULL, c_np1sec_delim);

  phased_message = base64_decode(encrypted_message);
  std::string temp = strtok(&phased_message[0], c_np1sec_delim);
  memcpy(session_id, temp.c_str(), temp.size());
  encrypted_message = strtok(NULL, c_np1sec_delim);

  phased_message = decrypt_message(base64_decode(encrypted_message));
  signed_message = strtok(&phased_message[0], c_np1sec_delim);
  signature = strtok(NULL, c_np1sec_delim);

  if (verify_message(signed_message, signature)) {
    signed_message = base64_decode(signed_message);
    std::string temp;
    temp = strtok(&signed_message[0], c_np1sec_delim);
    // TODO(bill): clarify session id check
//      if(session_id.compare(temp)) {
      sender_id = strtok(NULL, c_np1sec_delim);
      user_message = strtok(NULL, c_np1sec_delim);

      meta_message = strtok(NULL, c_np1sec_delim);
      unwrap_meta_message();
      std::string hash_string = strtok(NULL, c_np1sec_delim);
      memcpy(transcript_chain_hash, hash_string.c_str(), hash_string.size());
      nonce = strtok(NULL, c_np1sec_delim);
...
void np1secMessage::format_meta_message() {
  meta_message = "" + std::to_string(meta_only) + c_np1sec_delim;
...
void np1secMessage::unwrap_meta_message() {
  ...
  meta_only = atoi(strtok(&meta_message[0], c_np1sec_delim));
  std::string ustates = strtok(NULL, c_np1sec_delim);
  meta_load_flag = static_cast<np1secLoadFlag>(atoi(strtok(NULL, c_np1sec_delim)));
  meta_load = strtok(NULL, c_np1sec_delim);
}
...
std::string np1secMessage::format_sendable_message() {
  ...
  base_message = sid_string + c_np1sec_delim;
  base_message += sender_id + c_np1sec_delim;
  base_message += user_message + c_np1sec_delim;

  format_meta_message();
  base_message += meta_message + c_np1sec_delim;
  std::string hash_string(reinterpret_cast<char const*>(transcript_chain_hash));
  base_message += hash_string + c_np1sec_delim;
  base_message += nonce + c_np1sec_delim;

  base_message = base64_encode(base_message);

  signature = sign_message(base_message);
  base_message += signature + c_np1sec_delim;
  ...
  phased_message = "np1sec:O3" + phased_message + c_np1sec_delim;
